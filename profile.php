...
<body>
	<div class="row">
			...
			<?php
				if(isset($_POST['submit'])){
					...
					if($u_cover ==''){
						...
						$update = "update where users set user_cover = $u_cover.$random_number' where user_id = $user_id'";
		...
		<?php 
			if(isset($_POST['update'])){
				...
				if($u_image ==''){
					echo "<script>alert('Please select profile image on clicking on 
					your profile image')</script>";
					echo "<script>window.open('profile.php?u_id=$user_id','_self')</script>";
					exit();
				}else{
					move_uploaded_file($image_tmp, "users/$u_cover.$random_number");
					$update = "update where users set user_image = $u_image.$random_number' where user_id = $user_id'";
					$run = mysqli_query($con, $update);
	...
	<div class="row">
		...
		<div class="col-sm-6">
			...
			$get_posts = "select * from posts where user_id= '$u_id' ORDER by 1 DESC LIMIT "; 
			...
			while ($row_posts = mysqli_fetch_array($run_posts)){
				...
				$user_image = $row_useer['user_image'];
